 ...
 */
 var loudAndRich = function(richer, quiet) {
  // richer 包含多组贫富比较，但是不能得到完整的贫富排序，可以看作有向无环图
  // quiet 安静值排序
  // ans[x]=y 是找到比x富有的人中，quiet值最高的人
  // 所以关键点是：根据richer有向无环图 找出所有比x富有的人
  // 根据richer创建一个表，索引是0,,,i,,,n-1，值是比i富有的人的索引
  // 然后对表进行dfs，就能找到所有比i富有的人
  const n=quiet.length
  const richTable = new Array(n).fill(0).map(i=>new Set())
  ...
  const dfs=(cur)=>{
      let minIdx=cur
      for(let rich of richTable[cur].values()){
          const next=ans[cur]!==undefined?ans[cur]:dfs(rich)
          if(quiet[next]<quiet[minIdx]){
              minIdx=next
          }
      }
      ans[cur]=minIdx
  ...
  for(let i=0;i<n;i++){
      if(ans[i]===undefined) ans[i]=dfs(i)
  }
  return ans
};
// @lc code=end
