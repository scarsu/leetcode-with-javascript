...
var predictPartyVictory = function(senate) {
  let n=senate.length,ret=null

  // 权力状态,1代表有权力
  let active=new Array(n).fill(1)

  // 贪心：找到下一个离他最近的有权利的对手的索引
  const findFirstOpIdx = j => {
    for(let i=j+1;i<n;i++){
      if(senate[i] !== senate[j] && active[i] === 1) return i
    }
    for(let i=0;i<j;i++){
      if(senate[i] !== senate[j] && active[i] === 1) return i
    }
  ...
  while(ret===null){
    if(active[i]===1){
      let idx = findFirstOpIdx(i)
      if(idx > -1){
        // 找到对说 则禁用对手
        active[idx] = 0
      }else{
        // 找不到对手 则得出结果
        ret = senate[i] === 'D' ? 'Dire' : 'Radiant'
      }
    }
    i = i===n-1 ? 0 : i+1 // 下一轮 或 下一位
  }
